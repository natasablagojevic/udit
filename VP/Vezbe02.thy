...
lemma "(\<forall>x. leti x \<longrightarrow> krila x \<and> lagano x) \<and> (\<forall>x. pliva x \<longrightarrow> \<not> krila x)
...
lemma "(\<forall>x. decak x \<longrightarrow> igra x) \<and> (\<forall>x. devojcica x \<longrightarrow> igra x) 
  \<and> (\<forall>x. dete x \<longrightarrow> decak x \<or> devojcica x) \<longrightarrow> (\<forall>x. dete x \<longrightarrow> igra x)"
  by auto 
...
proof (intro allI impI)
  fix x
  assume "dete x"
  then have "decak x \<or> devojcica x" using assms(3) by auto
  then show "igra x"
  proof
    assume "decak x" 
    from assms(1) \<open>decak x\<close> show "igra x" by auto
  next 
    assume "devojcica x"
    from assms(2) \<open>devojcica x\<close> show "igra x" by auto
  qed
qed
...
\<longrightarrow> \<forall>x. greeks x \<longrightarrow> mortal x
*)
...
lemma Ferioque: 
  ...
  assumes "\<exists>x. reading x \<longrightarrow> homework x"
  shows "\<exists>x. reading x \<and> \<not> fun x"
proof
  assume "\<forall>x. \<not>(reading x \<and> \<not> fun x)"
  from this have "\<forall>x. reading x \<Longrightarrow> fun x" by auto
  obtain x where "reading x \<longrightarrow> homework x" using assms(2) by auto
  then have "reading x \<Longrightarrow> homework x" by auto
  have "\<not>(homework x  \<and> fun x)" using assms(1) by auto
  then have "\<not> homework x \<or> \<not> fun x" by auto 
  then have "\<not> fun x" using \<open>reading x \<Longrightarrow> homework x\<close> by auto
  show False using \<open>\<not> fun x\<close> by auto
(*
  moreover have "\<not> homework x \<or> \<not> fun x" using assms(1) by auto
  moreover have "\<not> fun x" using \<open>reading x \<Longrightarrow> homework x\<close> and \<open>reading x\<close> by auto
  ultimately show False using \<open>\<not> fun x\<close> and \<open>reading x\<close>  by auto
*)
qed
