...
primrec sum1_n :: "nat \<Rightarrow> nat" where 
  "sum1_n 0 = 0"
| "sum1_n (Suc n) = Suc n + sum1_n n"

value "sum1_n 5"

lemma "sum1_n n = (n * (n + 1)) div 2"
  by (induction n) auto
...
next 
  ...
  have "sum1_n (Suc n) = (Suc n) + sum1_n n" by auto
  also have "... = (Suc n) + (n * (n + 1)) div 2" using IH by auto
  also have "... = (2 * (Suc n)) div 2 + (n * (n + 1)) div 2" by auto
  also have "... = (2 * (n + 1)) div 2 + (n * (n + 1)) div 2" by auto
  also have "... = ((n + 1) * (n + 2)) div 2" by auto 
  also have "... = ((Suc n) * (n + 2)) div 2" by auto
