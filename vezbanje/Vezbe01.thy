...
definition sledbenik :: "nat \<Rightarrow> nat" where "sledbenik x = x + 1"
...
definition kvadrat :: "nat \<Rightarrow> nat" where "kvadrat x = x*x"
...
lemma "len lst = foldr (\<lambda> x. (+)1) lst 0"
...
----------------------
forall x . (laze(x) \<Rightarrow> krade(x)) and laze(x) \<Rightarrow> krade(x)
...
\<forall>x. radi x \<longrightarrow> ima x | trosi x 
...
lemma "(\<forall>x. radi x \<longrightarrow> ima x | trosi x) \<and> (\<forall>x. ima x \<longrightarrow> peva x) \<and> (\<forall>x. trosi x \<longrightarrow> peva x) \<longrightarrow> (\<forall>x. radi x \<longrightarrow> peva x) "
  by auto
...
\<not>(\<forall>x y. povredio x y \<and> voli x y)
\<exists> x y. povredio y x \<and> prijatelj y x
*)

lemma "(\<forall>x y. prijatelj x y \<longrightarrow> prijatelj y x) \<and> 
  (\<forall>x y. prijatelj x y \<longrightarrow> voli x y) \<and>
  (\<not>(\<exists>x y. povredio x y \<and> voli x y)) \<and> 
  (\<exists>y x.  povredio y x \<and> prijatelj y x) \<longrightarrow> False"
  by auto 
...
----------------------------------------------------
  laze(x) = x laze 
  krade(x) = x krade 
  uhvacen(x) = x je uhvacen u kradji
  zatvor(x) = x ide u zatvor 

----------------------------

\<forall>x. laze x \<longrightarrow> krade x
\<forall>x. (krade x \<and> uhvacen x) \<longrightarrow> zatvor x
laze AlKapone
uhvacen AlKapone
laze LakiLuciano
*)
