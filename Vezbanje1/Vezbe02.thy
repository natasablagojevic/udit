...
odgovara(c, t, n) = cipela u trenutku t odgovara nozi n
...
-----------------------------
(\<exists>c. \<forall>t. \<forall>n. odgovara c t n) \<longrightarrow> (\<forall>n. \<exists>c. \<exists>t. odgovara c t n) \<and> (\<forall>n. \<exists>c. \<exists>t. odgovara c t n)

*)

lemma "(\<exists>c. \<forall>t. \<forall>n. odgovara c t n) \<longrightarrow> (\<forall>n. \<exists>c. \<exists>t. odgovara c t n) \<and> (\<forall>n. \<exists>c. \<exists>t. odgovara c t n)"
  by auto 
...
proof 
  ...
  proof
    assume "dete x"
    from this have "decak x \<or> devojcica x" using assms(3) by auto 
    then show "igra x"
    proof
      assume "decak x"
...
\<forall>x y. \<exists>z. brat x y \<longrightarrow> roditelj z x \<and> roditelj z y 
\<forall>x y. roditelj x y \<longrightarrow> stariji x y 
\<exists>x y. brat x y 
\<not>(\<exists>x y. stariji x y)

*)

lemma "(\<forall>x y. \<exists>z. brat x y \<longrightarrow> roditelj z x \<and> roditelj z y) \<and> (\<forall>x y. roditelj x y \<longrightarrow> stariji x y) \<and> (\<exists>x y. brat x y) \<and> (\<not>(\<exists>x y. stariji x y)) \<longrightarrow> False"
  by auto 
