...
next
  case (Suc n)
  note ih = this 
  have "alternirajuca_suma (Suc n) = (-1)^(Suc n) * (2 * int(Suc n) - 1) + alternirajuca_suma n" by (auto simp add: algebra_simps)
  also have "... = (-1)^(Suc n) * (2 * int(Suc n) - 1) + (-1)^n * int n" using ih by (auto simp add: algebra_simps)
  also have "... = (-1)^(n+1) * (2 * int(n+1) - 1) + (-1)^n * int n" by (auto simp add: algebra_simps)
  also have "... = (-1)^(n+1) * (2 * int n + 2 - 1) + (-1)^n * int n" by (auto simp add: algebra_simps)
  also have "... = (-1)^(n+1) * (2 * int n + 1) + (-1)^n * int n" by (auto simp add: algebra_simps)
  also have "... = (-1) * (-1)^n * (2 * int n + 1) + (-1)^n * int n" by (auto simp add: algebra_simps)
  also have "... = (-1)^n * ((-1) * (2 * int n + 1) + int n)" by (auto simp add: algebra_simps)
  also have "... = (-1)^n * ((-2) * int n - 1 + int n)" by (auto simp add: algebra_simps)
...
next
  ...
  finally show ?case by (rule dvdI)
qed
...
next 
  case (Suc n)
  from this have "n^2 > 2*n +1" by simp 
  have "(Suc n)^2 = n^2 + 2 *n + 1" by simp 
  also have "... > 2*n + 1 + 2" using Suc by auto
