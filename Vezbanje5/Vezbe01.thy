...
fun kvadrat:: "nat \<Rightarrow> nat"
  where "kvadrat x = x*x"
...
lemma "(\<forall>x. laze x \<longrightarrow> krade x) \<and> (\<exists>x. laze x) \<longrightarrow> (\<exists>x. krade x)"
proof
  assume "(\<forall>x. laze x \<longrightarrow> krade x) \<and> (\<exists>x. laze x)"
  from this have "\<forall>x. laze x \<longrightarrow> krade x" "\<exists>x. laze x" by auto 
  from \<open>\<exists>x. laze x\<close> obtain x where "laze x" by auto 
  from this \<open>\<forall>x. laze x \<longrightarrow> krade x\<close> have "krade x" by auto 
  from this show "\<exists>x. krade x" by auto
qed
...
lemma "(\<forall>x. radi x \<longrightarrow> ima x \<or> trosi x) \<and> (\<forall>x. ima x \<longrightarrow> peva x) \<and> (\<forall>x. trosi x \<longrightarrow> peva x) \<longrightarrow> (\<forall>x. radi x \<longrightarrow> peva x)"
proof
  assume A: "(\<forall>x. radi x \<longrightarrow> ima x \<or> trosi x) \<and> (\<forall>x. ima x \<longrightarrow> peva x) \<and> (\<forall>x. trosi x \<longrightarrow> peva x)"
  then have H1: "\<forall>x. radi x \<longrightarrow> ima x \<or> trosi x" by auto 
  from A have H2: "\<forall>x. ima x \<longrightarrow> peva x" by auto 
  from A have H3: "\<forall>x. trosi x \<longrightarrow> peva x" by auto
  show "\<forall>x. radi x \<longrightarrow> peva x"
  proof
    fix x 
    assume "radi x"
    from H1 have "radi x \<longrightarrow> ima x \<or> trosi x" by auto 
    from this \<open>radi x\<close> have "ima x \<or> trosi x" by auto 
    then show "peva x"
    proof
      assume "ima x"
      from H2 have "ima x \<longrightarrow> peva x" by auto 
      from this \<open>ima x\<close> show "peva x" by auto 
    next 
      assume "trosi x"
      from H3 have "trosi x \<longrightarrow> peva x" by auto
      from this \<open>trosi x\<close> show "peva x" by auto
...
\<not>(\<exists>x. \<exists>y. povredio x y \<and> voli x y)
\<exists>y. \<exists>x. povredio y x \<and> prijatelj y x

*)

lemma "(\<forall>x y. prijatelj x y \<longrightarrow> prijatelj y x) \<and> (\<forall>x y. prijatelj x y \<longrightarrow> voli x y) \<and> (\<not>(\<exists>x y. povredio x y \<and> voli x y)) \<and> (\<exists>y x. prijatelj y x  \<and> povredio y x) \<longrightarrow> False"
  by auto 
