...
proof
  ...
  proof
    ...
    proof
      assume "x \<in> A"
      then have "y \<in> f ` A" using \<open>f x = y\<close> by auto 
      then show "y \<in> f ` A \<union> f ` B" by auto 
    next 
      assume "x \<in> B"
      then have "y \<in> f ` B" using \<open>f x = y\<close> by auto 
      then  show "y \<in> f ` A \<union> f ` B" by auto 
    qed
  qed
next 
  ...
  proof
    ...
    proof
      ...
      then  show ?thesis using \<open>f x = y\<close> by auto  
    next 
      assume "y \<in> f ` B"
...
next 
  ...
  proof
    fix y
    assume "y \<in> B"
    from assms obtain x where "f x = y" using surj_def by blast
...
proof (rule injI)
  ...
  with assms(1) have "g x = g y" unfolding inj_def by simp
  with assms(2) have "x = y" unfolding inj_def by simp 
...
next 
  ...
  with assms have "y = x" unfolding inj_def by simp
  with \<open>y \<in> A\<close> show "x \<in> A" by simp
qed
...
proof
  ...
  proof
    ...
    then obtain y where "y \<in> A" "f x = f y" by auto 
    with assms have "y = x" unfolding inj_def by auto 
    with \<open>y \<in> A\<close> show "x \<in> A" by auto 
  qed
next 
...
proof (unfold surj_def, intro allI)
  fix y
  from assms(1) obtain x where "f x = y" unfolding surj_def by metis
  from assms(2) obtain z where "g z = x" unfolding surj_def by metis
  have "(f \<circ> g) z = f (g z)" by auto 
  also have "... = f x" using \<open>g z = x\<close> by auto 
  also have "... = y" using \<open>f x = y\<close> by auto 
  finally have "(f \<circ> g) z = y" .
  then show "\<exists>z. (f \<circ> g) z = y" by auto
qed
