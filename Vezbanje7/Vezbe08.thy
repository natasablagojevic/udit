...
fun saberi:: "prirodni \<Rightarrow> prirodni \<Rightarrow> prirodni" (infixl "\<oplus>" 100) where
  "\<zero> \<oplus> b = b"
| "(Sled a) \<oplus> b = Sled (a \<oplus> b)"

fun saberi1:: "prirodni \<Rightarrow> prirodni \<Rightarrow> prirodni" 
  where "saberi1 Nula y = y"
  | "saberi1 (Sled x) y = Sled (saberi x y)"
