...
next 
  ...
  proof
    ...
    then have "x1 = x2" using \<open>f x1 = y\<close> \<open>f x2 = y\<close> assms  by (simp add: inj_def)
...
next 
  ...
  proof
    fix y
    assume "y \<in> B"
    then obtain x where "f x = y" using surj_def by (metis assms)
...
proof (rule injI)
  ...
  with assms(1) have "g x = g y" unfolding inj_def by auto 
  with assms(2) show "x = y" unfolding inj_def by auto
qed
...
proof
  ...
  proof
    ...
    then obtain y where "y \<in> A" "f x = f y" by auto 
    from this assms have "x = y" unfolding inj_def by auto 
    then show "x \<in> A" using \<open>y \<in> A\<close> by auto 
  qed
next 
